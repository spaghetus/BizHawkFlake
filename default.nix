{
  ...
  # source
  hawkSourceInfoDevBuild ? let
    # called "dev build", but you could pass whatever branch and commit you want here
  ...
  in {
    ...
    }; # source derivation; did have filter here for speed, but it wasn't faster and it wasn't correct and it couldn't be made correct and I'm mad
  },
  # makedeps
  dotnet-sdk_8 ? pkgs.dotnet-sdk_8,
  dotnet-sdk_6 ? pkgs.dotnet-sdk_6,
  dotnet-sdk_5 ? let
    result = builtins.tryEval (pkgs.dotnet-sdk_5 or builtins.throw "no .NET 5 SDK");
  in
    if result.success
    then result.value
  ...
  kate ? pkgs.kdePackages.kate, #.overrideAttrs (oldAttrs: {
  #   patches =
  #     (oldAttrs.patches or [])
  #     ++ [
  #       (fetchpatch {
  ...
  */
  ...
  inherit
    ...
    ;
  launchScriptsFor = bizhawkAssemblies: isManualLocalBuild:
    import Dist/launch-scripts.nix {
      inherit
        lib
        writeShellScript
        writeText
...
in
  combined
  // lib.listToAttrs (lib.concatLists (builtins.map
    (f: [
      {
        name = f "latest-bin";
        value = combined.${f "${latestVersionFrag}-bin"};
      }
      {
        name = f "latest";
        value = combined.${f latestVersionFrag};
      }
    ])
